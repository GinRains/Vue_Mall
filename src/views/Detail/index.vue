        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="skuSaleAttr in skuSaleAttrList" :key="skuSaleAttr.id">
                <dt class="title">{{skuSaleAttr.saleAttrName}}</dt>
                <dd changepirce="0" :class="{active: skuSaleAttrValue.isChecked === '1'}"
                    @click="changeIsChecked(skuSaleAttr.spuSaleAttrValueList, skuSaleAttrValue)"
                    v-for="skuSaleAttrValue in skuSaleAttr.spuSaleAttrValueList" :key="skuSaleAttrValue.id" >{{skuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="goodsNum" @change="goodsNum = +$event.target.value > 0 ? +$event.target.value : 1">
                <a href="javascript:" class="plus" @click="goodsNum++">+</a>
                <a href="javascript:" class="mins" @click="goodsNum > 1 ? goodsNum-- : 1">-</a>
              </div>
              <div class="add">
                <a href="javascript:;" @click="addToCart(skuInfo.id, goodsNum)">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      getGoodsInfo() {
        ...
        this.$store.dispatch("getGoodsDetailInfo", skuId)
      },
      changeIsChecked(goods, currentGoods) {
        goods.forEach(item => {
          item.isChecked = '0'
        })
        currentGoods.isChecked = '1'
      },
      async addToCart(skuId, skuNum) {
        const goodsInfo = { skuId, skuNum }
        try {
          const result = await this.$store.dispatch("addToCart", goodsInfo)
          sessionStorage.setItem("SKUINFO_KEY", JSON.stringify(this.skuInfo))
          alert(result)
          this.$router.push(`/addcart/${goodsInfo.skuNum}`)
        }catch(error) {
          alert(error.message)
        }
      }
