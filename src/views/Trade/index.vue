<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div @click="changeDefaultAddress(index)" class="address clearFix" v-for="(userAddress, index) in tradeInfo.userAddressList" :key="userAddress.id">
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="goods in tradeInfo.detailArrayList" :key="goods.skuId">
          <li>
            <img style="width: 82px; height: 82px;" :src="goods.imgUrl" alt="">
          </li>
          <li>
            <p>
              {{goods.skuName}}
  ...
  export default {
    ...
    methods: {
      ...
      changeDefaultAddress(index) {
        const {userAddressList} = this.tradeInfo
        userAddressList.forEach(item => item.isDefault = "0")
        userAddressList[index].isDefault = "1"
      },
      async toPay() {
        const defaultUserAddress = this.tradeInfo.userAddressList.reduce((prev, curr) => {
          if(curr.isDefault === "1") {
            prev = curr
          }
          return prev
        ...
        const tradeInfo = {
          consignee: defaultUserAddress.consignee,
          consigneeTel: defaultUserAddress.phoneNum,
          deliveryAddress: defaultUserAddress.userAddress,
          paymentWay: "ONLINE",
          orderComment: "",
        ...
        try {
          const res = await this.$store.dispatch("getMyPayInfo", {tradeInfo, tradeNo: this.tradeInfo.tradeNo})
          alert(res)
          this.$router.push({
            path: '/pay'
          })
