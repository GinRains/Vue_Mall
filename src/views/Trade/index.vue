  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(userAddress, index) in userAddressList" :key="userAddress.id">
        ...
        <p @click="updateUserAddress(index)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="goodsInfo in goodsInfoList" :key="goodsInfo.skuId">
          <li>
            <img :src="goodsInfo.imgUrl" style="width: 82px; height: 82px;">
          </li>
          <li>
            <p>
              {{goodsInfo.skuName}}
          ...
          <li>
            <h3>￥{{goodsInfo.orderPrice}}.00</h3>
          </li>
          <li>X{{goodsInfo.skuNum}}</li>
          <li>有货</li>
        </ul>
      </div>
      <div class="bbs">
...
<script>
  export default {
    ...
    methods: {
      async getTradeInfo() {
        const response = await this.$API.reqOrderInfo()
        if(response.code === 200) {
          this.tradeInfo = response.data
        }
      },
      updateUserAddress(index) {
        this.tradeInfo.userAddressList.forEach(item => item.isDefault = "0")
        this.tradeInfo.userAddressList[index].isDefault = "1"
      },
      async submitOrder() {
        const orderOptions = {
          ...
          "orderDetailList": this.goodsInfoList
        }
        const response = await this.$API.reqOrderList(this.tradeInfo.tradeNo, orderOptions)
        if(response.code === 200) {
          this.$router.push(`/pay?orderNo=${response.data}`)
          this.$router.push(`/pay?tradeNo=${response.data}`)
        }
      }
    },
    computed: {
